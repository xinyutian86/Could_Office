...
<body>
	...
	<%
		ArrayList newslist=(ArrayList)news.getCheckerNews();
	...
	%>
		...
		<td class="editor"><%= m.get("editor") %></td>
		<td class="sender"><%= m.get("sender") %></td>
		<td class="date"><%= m.get("date") %></td><td style="width: 500px;" class="content"><%= m.get("content") %></td>
		<td><a href="admin_edit.jsp?id=<%=m.get("id")%>&sender=<%=gp%>&who=<%=name%>">修改</a>&nbsp;<a href="refuse.jsp?id=<%=m.get("id")%>&sender=<%=gp%>&who=<%=name%>" onclick="return confirm('确定要拒收吗?')">拒收</a>&nbsp;<a href="pass.jsp?id=<%=m.get("id")%>&sender=<%=gp%>&who=<%=name%>" onclick="return confirm('确定要通过吗?')">通过</a></td></tr>
	<%
		}
	%>
	</table>
