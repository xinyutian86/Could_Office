...
<html lang="zh-CN">
    ...
    color: #444;
		...
		<style id='theme-skin-inline-css' type='text/css'>
			...
			.sc_testimonials_style_1 .flex-direction-nav a:active,
			.pagination .flex-direction-nav a:active .sc_scroll_controls .flex-direction-nav a:hover,
			...
			<div class="boxedWrap">
				...
				<div id="topOfPage" class="topTabsWrap">
					<div class="main">
						...
						<h1 class="pageTitle h1">在线点歌</h1></div>
				</div>
...
<table class="bordered">
    ...
    <%
		ArrayList newslist=(ArrayList)news.allMusic();
		for(Object o:newslist){
			Map m=(HashMap)o;
	%>
		
		<tr id="table" class="admin"><td style="width: 50px;" class="bianhao"><%= m.get("from") %></td><td class="title"><%= m.get("phone") %></td>
		<td class="sender"><%= m.get("song") %></td>
		<td class="sender"><%= m.get("singer") %></td>
		<td class="author"><%= m.get("to") %></td>
		<td class="sender"><%= m.get("date") %></td>
		<td class="date"><%= m.get("state") %></td>
		<td><a href="music_do.jsp?id=<%=m.get("id")%>" onclick="return confirm('确定已经播出了吗?')">已完成</a></td></tr>
	<%
		}
	%>
</table>
						</article>
						<article class="bg_post post_format_standard postLeft odd viewmore post-23 post type-post status-publish format-standard hentry category-6">
